...
public class RenderableMenuItem implements ActionListener {
	...
	public RenderableMenuItem(Item item, int x, int y, JPanel panel) {
		...
		BufferedImage img = new BufferedImage(iconImg.getIconWidth(), iconImg.getIconHeight(),
				BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = img.createGraphics();
		int xShift = 0;
		Point[] itemPoints = { new Point(50 + xShift, 0), new Point(250 + xShift, 0), new Point(300 + xShift, 25),
				new Point(250 + xShift, 50), new Point(50 + xShift, 50), new Point(0 + xShift, 25) };
		if (item.getType() == ItemType.BLUEPRINT) {
			Color[] colors = { new Color(212 / 6, 175 / 6, 55 / 6),
					new Color((212 * 2) / 5, (175 * 2) / 5, (55 * 2) / 5) };
		...
		Point[] points = { new Point(tx, ty), new Point(tx + image.getWidth() + 10, ty),
				new Point(tx + image.getWidth() + 10, ty + image.getHeight() + 10),
				new Point(tx, ty + image.getHeight() + 10) };
	...
	public void draw(Graphics2D g, int JPanelX, int JPanelY) {
		...
		if (item != null) {
			if (item.getType() == ItemType.CONSUMABLE) {
				...
				FontMetrics metrics = g.getFontMetrics();
				g.drawString(count.substring(0, count.indexOf('.')),
						image.getWidth() + x - metrics.stringWidth(count.substring(0, count.indexOf('.'))),
						y + metrics.getHeight());
			...
			if (showDescription) {
				...
				if (item.getType() == ItemType.BLUEPRINT) {
					...
					if (blueprint.getProduct().getType() == ItemType.CONSUMABLE) {
						g.drawString(
								blueprint.getProduct().getName() + " x"
										+ ((Consumable) blueprint.getProduct()).getCount(),
								2180 - metrics.stringWidth(blueprint.getProduct().getName() + " x"
										+ ((Consumable) blueprint.getProduct()).getCount()) / 2,
								340);
					} else if (blueprint.getProduct().getType() == ItemType.MATERIAL) {
						g.drawString(
								blueprint.getProduct().getName() + " x"
										+ ((Material) blueprint.getProduct()).getCount(),
								2180 - metrics.stringWidth(blueprint.getProduct().getName() + " x"
										+ ((Material) blueprint.getProduct()).getCount()) / 2,
								340);
					} else {
						g.drawString(blueprint.getProduct().getName() + " x1",
								2180 - metrics.stringWidth(blueprint.getProduct().getName() + " x1") / 2, 340);
	...
	@Override
	// Stuff it does when buttons are pressed. Namely fire actions and updating
	// inventory
