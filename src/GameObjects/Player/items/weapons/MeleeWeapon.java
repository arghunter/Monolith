...
public class MeleeWeapon extends Weapon {
	...
	// Constructors
	public MeleeWeapon(String name, int tier, int damage, int range, int attackSpeed, StatusEffect effect,
			double duration, double statusChance, double sweepAngle) {
	...
	public void drawWeapon(Player player, Graphics2D g) {
		...
		double angle = player.getAngle() - sweepAngle / 2 + sweepAngle
				* (9 * (System.currentTimeMillis() - super.getLastAttack())) / super.getCurrentAttackDelay();
		if ((System.currentTimeMillis() - super.getLastAttack()) / super.getCurrentAttackDelay() > 0.1)

		{

			angle = player.getAngle() + sweepAngle - (2 * sweepAngle
					* (((System.currentTimeMillis() - super.getLastAttack())) / super.getCurrentAttackDelay()));
		}
		...
		img.move((int) (player.getX() - (img.getX()) + 64 * Math.cos(angle)),
				(int) (player.getY() - (img.getY() - 64 * Math.sin(angle))));
		img.setRotation(angle);

		graphic = g;

		g.fillArc((int) player.getX() - super.getRange(), (int) player.getY() - super.getRange(), super.getRange() * 2,
				super.getRange() * 2, (int) ((-player.getAngle() - sweepAngle / 2) * 180 / Math.PI),
				(int) (sweepAngle * 180 / Math.PI));
		img.drawImage(g);
	}
		...
		if (canFire()) {
			...
			for (Mob m : mobs) {
				...
				if (m != null && this.euclidDist(m.getX(), m.getY(), player.getX(), player.getY()) < super.getRange()) {
					Arc2D.Double attackArc = new Arc2D.Double((int) player.getX() - super.getRange(),
						...
						synchronized (mobs) {
							if (totalStatus < 3 && Math.random() <= getChance()) {
								Damage dmg = new Damage(
										(int) (super.getDamage()
												* (Math.log10(player.getStats()[4] + player.getStats()[8]) + 1)),
										getEffect(), getDuration(), m, player, mobs);
								totalStatus++;
							} else {
								Damage dmg = new Damage(
										(int) (super.getDamage()
												* (Math.log10(player.getStats()[4] + player.getStats()[8]) + 1)),
										StatusEffect.NONE, 0, m, player, (mobs));
